  ...
  async function saveProfile(){
    ...
    try {
      ...
      if(myProfileId){
        ...
        if(error)throw error;
        // Szövegmezők külön frissítés (schema cache megkerülése)
        await supabase.from("profiles").update({instagram:myProfile.instagram||null, leiras:myProfile.leiras||null}).eq("id",myProfileId);
        setDbProfiles(prev=>prev.map(p=>p.id===myProfileId?{...p,...myProfile,avatarUrl:avatarUrl||""}:p));
      } else {
        const {data,error}=await supabase.from("profiles").insert([payload]).select().single();
        if(error)throw error;
        // Szövegmezők külön frissítés
        await supabase.from("profiles").update({instagram:myProfile.instagram||null, leiras:myProfile.leiras||null}).eq("id",data.id);
