      ...
      <div className="profile-card-header">
        <div style={{display:"flex",alignItems:"center",gap:"0.8rem"}}>
          <div className="avatar" style={{position:"relative"}}>
            ...
            {profile.maEdzek&&<span style={{position:"absolute",bottom:-2,right:-2,fontSize:"0.8rem"}}>🔥</span>}
          </div>
...
// ── Swipe Mode ────────────────────────────────────────────────────────────────
function SwipeMode({ profiles, user, onLogin, myProfileId }) {
    ...
    try {
      ...
      if(action === "like") {
        ...
        const {data} = await supabase.from("swipes")
          ...
          .eq("action","like")
          .maybeSingle();
        if(data) setMatch(currentProfile);
      }
    } catch(e){}

    setSwipedIds(prev=>new Set([...prev, currentProfile.id]));
    setTimeout(()=>{
      setAnimating(null);
  ...
  function onMove(clientX, clientY) {
    ...
    setDrag(d=>({...d, x:clientX-startPos.current.x, y:clientY-startPos.current.y}));
  }
  function onEnd() {
    if(!drag.dragging) return;
    if(drag.x > 80) doSwipe("like");
    else if(drag.x < -80) doSwipe("nope");
  ...
  const cardStyle = {
    transform: animating === "right" ? "translateX(120%) rotate(20deg)" :
               animating === "left" ? "translateX(-120%) rotate(-20deg)" :
               `translateX(${drag.x}px) translateY(${drag.y*0.3}px) rotate(${rotate}deg)`,
    transition: animating ? "transform 0.35s ease" : drag.dragging ? "none" : "transform 0.3s ease",
  ...
  if(!currentProfile || current >= queue.length) return (
    <div style={{textAlign:"center",padding:"3rem"}}>
      ...
      <div style={{color:"var(--muted)",marginBottom:"1.5rem"}}>Gyere vissza később amikor új profilok érkeznek.</div>
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",paddingTop:"1rem"}}>
      {/* Progress */}
      <div style={{color:"var(--muted)",fontSize:"0.82rem",marginBottom:"1rem"}}>{progress}</div>

      {/* Card stack */}
      <div style={{position:"relative",width:"100%",maxWidth:380,height:520,marginBottom:"1.5rem"}}>
        {/* Next card shadow */}
        {queue[current+1]&&(
        ...
        {/* Current card */}
        <div ref={cardRef} style={{position:"absolute",inset:0,zIndex:1,...cardStyle,userSelect:"none"}}
          ...
          {/* Like glow */}
          {drag.x > 30 && (
            <div style={{position:"absolute",inset:0,borderRadius:20,border:"3px solid #4ade80",boxShadow:"0 0 40px rgba(74,222,128,0.4)",zIndex:10,pointerEvents:"none",opacity:Math.min(opacity*2,1)}}>
              <div style={{position:"absolute",top:24,left:24,background:"#4ade80",color:"#000",fontFamily:"var(--font-display)",fontSize:"1.8rem",letterSpacing:2,padding:"0.3rem 0.8rem",borderRadius:8,transform:"rotate(-15deg)"}}>LIKE 💚</div>
            </div>
          )}
          {/* Nope glow */}
          {drag.x < -30 && (
            <div style={{position:"absolute",inset:0,borderRadius:20,border:"3px solid #ff4444",boxShadow:"0 0 40px rgba(255,68,68,0.4)",zIndex:10,pointerEvents:"none",opacity:Math.min(opacity*2,1)}}>
              <div style={{position:"absolute",top:24,right:24,background:"#ff4444",color:"#fff",fontFamily:"var(--font-display)",fontSize:"1.8rem",letterSpacing:2,padding:"0.3rem 0.8rem",borderRadius:8,transform:"rotate(15deg)"}}>NOPE ❌</div>
            </div>
          )}
          ...
          <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,overflow:"hidden",height:"100%",display:"flex",flexDirection:"column"}}>
            {/* Avatar */}
            <div style={{height:280,background:"var(--surface2)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,position:"relative"}}>
              {currentProfile.avatarUrl
                ? <img src={currentProfile.avatarUrl} alt={currentProfile.nev} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                : <span style={{fontSize:"6rem"}}>{currentProfile.avatar}</span>
              }
              {currentProfile.maEdzek&&(
                <div style={{position:"absolute",top:12,right:12,background:"rgba(255,107,53,0.9)",color:"#fff",fontSize:"0.75rem",fontWeight:700,padding:"0.25rem 0.6rem",borderRadius:100}}>🔥 Ma edzek</div>
              )}
            </div>
            {/* Info */}
            <div style={{padding:"1.25rem",flex:1,display:"flex",flexDirection:"column",gap:"0.5rem"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline"}}>
                <span style={{fontFamily:"var(--font-display)",fontSize:"1.8rem",letterSpacing:1}}>{currentProfile.nev}</span>
                <span style={{color:"var(--muted)",fontSize:"1rem"}}>{currentProfile.kor} év</span>
              </div>
              <div style={{color:"var(--muted)",fontSize:"0.85rem"}}>{currentProfile.varos}, {currentProfile.megye}</div>
              {badges.length>0&&(
                <div style={{display:"flex",flexWrap:"wrap",gap:"0.3rem"}}>
                  {badges.map(b=><span key={b.label} style={{fontSize:"0.68rem",padding:"0.15rem 0.5rem",borderRadius:100,background:`${b.color}22`,color:b.color,border:`1px solid ${b.color}44`,fontWeight:600}}>{b.icon} {b.label}</span>)}
                </div>
              )}
              <div style={{display:"flex",flexWrap:"wrap",gap:"0.3rem"}}>
                {currentProfile.edzesTipus.slice(0,3).map(t=><span key={t} className="tag active" style={{fontSize:"0.72rem"}}>{t}</span>)}
              </div>
              {currentProfile.leiras&&<div style={{color:"#aaa",fontSize:"0.82rem",lineHeight:1.5,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical"}}>{currentProfile.leiras}</div>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:"1.5rem",alignItems:"center"}}>
        <button onClick={()=>doSwipe("nope")} style={{width:64,height:64,borderRadius:"50%",border:"2px solid #ff4444",background:"transparent",fontSize:"1.8rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}
          onMouseEnter={e=>e.target.style.background="rgba(255,68,68,0.15)"}
          onMouseLeave={e=>e.target.style.background="transparent"}>❌</button>
        <button onClick={()=>doSwipe("like")} style={{width:72,height:72,borderRadius:"50%",border:"2px solid #4ade80",background:"transparent",fontSize:"2rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}
          onMouseEnter={e=>e.target.style.background="rgba(74,222,128,0.15)"}
          onMouseLeave={e=>e.target.style.background="transparent"}>💚</button>
      </div>
      <div style={{color:"var(--muted)",fontSize:"0.75rem",marginTop:"0.75rem"}}>Húzd jobbra a like-hoz, balra a skip-hez</div>
      ...
      {match&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.9)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:"1rem"}}>
          <div style={{background:"var(--surface)",border:"2px solid var(--neon)",borderRadius:20,padding:"2.5rem",textAlign:"center",maxWidth:360,width:"100%",boxShadow:"0 0 60px rgba(200,242,48,0.3)"}}>
            <div style={{fontSize:"3rem",marginBottom:"0.5rem"}}>💥</div>
            <div style={{fontFamily:"var(--font-display)",fontSize:"2.5rem",letterSpacing:3,color:"var(--neon)",marginBottom:"0.5rem"}}>MATCH!</div>
            <div style={{color:"#ccc",marginBottom:"1.5rem",lineHeight:1.6}}>
              Te és <strong style={{color:"var(--neon)"}}>{match.nev}</strong> egymást választottátok!
            </div>
            {match.instagram&&(
              <button className="btn btn-neon" style={{width:"100%",justifyContent:"center",marginBottom:"0.75rem"}}
...
export default function GymBro() {
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_e,session)=>{
      ...
      if(session?.user) { loadMatches(session.user.id); loadFavorites(session.user.id); loadBlocks(session.user.id); }
  ...
  async function saveProfile(){
    ...
    try {
      ...
      if(myProfileId){
        ...
        if(error)throw error;
        await supabase.from("profiles").update({leiras:myProfile.leiras||null,instagram:myProfile.instagram||null}).eq("id",myProfileId).eq("user_id",user.id);
        setDbProfiles(prev=>prev.map(p=>p.id===myProfileId?{...p,...myProfile,avatarUrl:avatarUrl||""}:p));
      } else {
        const {data,error}=await supabase.from("profiles").insert([payload]).select().single();
        if(error)throw error;
        setMyProfileId(data.id);
        await supabase.from("profiles").update({leiras:myProfile.leiras||null,instagram:myProfile.instagram||null}).eq("id",data.id).eq("user_id",user.id);
        setDbProfiles(prev=>[{...myProfile,id:data.id,kor:parseInt(myProfile.kor)||25,avatar:"👤",online:true,userId:user.id,avatarUrl:avatarUrl||"",leiras:myProfile.leiras,instagram:myProfile.instagram},...prev]);
      }
        ...
        <div className="main">
          ...
          {tab==="kereses"&&(
            <>
              ...
              {swipeMode?(
                <SwipeMode profiles={profiles.filter(p=>!myBlocks.includes(p.id))} user={user} onLogin={()=>setShowAuth(true)} myProfileId={myProfileId}/>
              ):(<>
              {/* Dinamikus banner */}
              {myProfile.megye && (
