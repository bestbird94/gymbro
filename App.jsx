...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .avatar-upload { position: relative; display: block; border-radius: 50%; overflow: hidden; }
  .avatar-overlay {
    position: absolute; inset: 0; background: rgba(0,0,0,0.6); border-radius: 50%;
    display: flex; align-items: center; justify-content: center;
    font-size: 0.65rem; color: #fff; opacity: 0; transition: opacity 0.2s; text-align: center;
  }
  ...
  .avatar-overlay {
    ...
    display: flex; align-items: center; justify-content: center;
    font-size: 0.65rem; color: var(--neon); opacity: 0; transition: opacity 0.2s;
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="divider" />
        <div className="flex gap-2" style={{gap:"0.75rem"}}>
          <button className="btn btn-neon" style={{flex:1}} onClick={()=>{onMessage&&onMessage(profile);onClose();}}>💬 Üzenet küldése</button>
          <button className="btn btn-outline btn-sm">⭐ Mentés</button>
        </div>
      </div>
    </div>
...
function MessagesTab({ user, profiles, onLogin, initialTarget }) {
  ...
  async function openThread(otherUserId) {
    ...
    // Mark as read
    await supabase.from("messages").update({ read: true }).eq("to_user_id", user.id).eq("from_user_id", otherUserId);
  }
  ...
  async function sendMessage() {
    ...
    await supabase.from("messages").insert([{
      from_user_id: user.id, to_user_id: activeThread,
  ...
  const threads = [...new Map(inbox.map(m => {
    ...
    return [otherId, { otherId, lastMsg: m.content, unread: !m.read && m.to_user_id === user?.id }];
  })).values()];

  const getPartnerName = (id) => profiles.find(p => p.userId === id)?.nev || id?.slice(0,8) + "...";

  if (!user) return (
    <div className="card" style={{textAlign:"center",padding:"3rem"}}>
      <div style={{fontSize:"2.5rem",marginBottom:"0.75rem"}}>💬</div>
      <div className="text-muted" style={{marginBottom:"1rem"}}>Jelentkezz be az üzenetek megtekintéséhez!</div>
  ...
  return (
    <div>
      <div style={{marginBottom:"1.5rem"}}>
        <h1 className="font-display" style={{fontSize:"2.2rem",letterSpacing:2,marginBottom:"0.4rem"}}>ÜZENETEK</h1>
        <p className="text-muted">Edzőpartnereiddel folytatott beszélgetések</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:"1rem",minHeight:400}}>
        {/* Thread list */}
        <div className="card" style={{padding:0,overflow:"hidden"}}>
          {loading && <div style={{padding:"1rem",textAlign:"center"}}><span className="spinner" style={{width:20,height:20,margin:"0 auto"}}/></div>}
          ...
          {threads.map(t => (
            <div key={t.otherId} onClick={()=>openThread(t.otherId)}
              style={{padding:"1rem",borderBottom:"1px solid var(--border)",cursor:"pointer",
                background: activeThread===t.otherId ? "var(--neon-dim)" : "transparent",
                transition:"background 0.15s"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:600,fontSize:"0.9rem"}}>{getPartnerName(t.otherId)}</span>
                {t.unread && <span style={{width:8,height:8,borderRadius:"50%",background:"var(--neon)",display:"inline-block"}}/>}
              </div>
              <div style={{fontSize:"0.78rem",color:"var(--muted)",marginTop:"0.2rem",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.lastMsg}</div>
            </div>
          ))}
        </div>
        {/* Thread view */}
          ...
          {!activeThread ? (
            ...
            <>
              <div style={{flex:1,overflowY:"auto",padding:"1rem",display:"flex",flexDirection:"column",gap:"0.5rem"}}>
                {threadMessages.map(m => (
                  <div key={m.id} style={{display:"flex",justifyContent: m.from_user_id===user.id ? "flex-end":"flex-start"}}>
                    <div style={{maxWidth:"70%",padding:"0.6rem 0.9rem",borderRadius:12,
              ...
              <div style={{padding:"1rem",borderTop:"1px solid var(--border)",display:"flex",gap:"0.5rem"}}>
                <input className="form-input" placeholder="Üzenet..." value={newMsg}
...
export default function GymBro() {
  ...
  async function uploadAvatar(file) {
    ...
    try {
      ...
      // Ha már van profilunk, frissítjük az avatar_url-t
      if (myProfileId) {
        await supabase.from("profiles").update({ avatar_url: data.publicUrl }).eq("id", myProfileId);
        setProfiles(prev => prev.map(p => p.id === myProfileId ? {...p, avatarUrl: data.publicUrl} : p));
      }
    } catch(e) {
      setSaveError("Képfeltöltési hiba: " + e.message);
    }
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          const mapped = data.map(p => ({
            ...
            gym: p.gym, edzesTipus: p.edzes_tipus || [], eroSzint: p.ero_szint,
            napok: p.napok || [], idopontok: p.idopontok || [], leiras: p.leiras,
            avatar: "👤", online: p.online ?? true, userId: p.user_id, avatarUrl: p.avatar_url || ""
          }));
          setProfiles([...mapped, ...DEMO_PROFILES]);
          // Ha be van jelentkezve, megkeressük a saját profil id-jét
          const session = await supabase.auth.getSession();
  ...
  async function uploadAvatar(file) {
    ...
    try {
      ...
      if (myProfileId) {
        ...
        setProfiles(prev => prev.map(p => p.id === myProfileId ? {...p, avatarUrl: url} : p));
      }
  ...
  async function saveProfile() {
    ...
    try {
      const { data, error } = await supabase.from("profiles").insert([{
        ...
        edzes_tipus: myProfile.edzesTipus, ero_szint: myProfile.eroSzint,
        napok: myProfile.napok, idopontok: myProfile.idopontok,
        leiras: myProfile.leiras, online: true, user_id: user.id, avatar_url: avatarUrl || null
      }]).select().single();
      if (error) throw error;
      const newP = { ...myProfile, id: data.id, kor: parseInt(myProfile.kor)||25, avatar: "👤", online: true, userId: user.id, avatarUrl: avatarUrl || "" };
      setProfiles(prev => [newP, ...prev]);
      setMyProfileId(data.id);
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
    ...
    <>
      ...
      <div className="app">
        ...
        <nav className="nav">
          <div className="nav-logo"><GymBroLogo /></div>
          <div className="nav-tabs">
            {[["kereses","🔍 Keresés"],["ai","🤖 AI Ajánló"],["profil","👤 Profilom"],["uzenetek","💬 Üzenetek"]].map(([k,l]) => (
              <button key={k} className={`nav-tab ${tab===k?"active":""}`} onClick={()=>setTab(k)}>{l}</button>
            ))}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"0.5rem"}}>
            ...
            <>
              ...
              <div className="card">
                ...
                <div style={{display:"flex",alignItems:"center",gap:"1.2rem",marginBottom:"1.5rem",paddingBottom:"1.5rem",borderBottom:"1px solid var(--border)"}}>
                  <label className="avatar-upload" style={{width:72,height:72,cursor: user ? "pointer":"default"}}>
                    <div className="avatar lg">
                      {avatarUrl
                    ...
                    {user && (
                      <div className="avatar-overlay">
                        {avatarUploading ? <span className="spinner" style={{width:16,height:16}}/> : "📷 Kép"}
                      </div>
                    )}
                    {user && <input type="file" accept="image/*" style={{display:"none"}}
                      onChange={e=>e.target.files[0]&&uploadAvatar(e.target.files[0])} />}
                  </label>
                  <div>
                    <div style={{fontWeight:600,marginBottom:"0.2rem"}}>Profilkép</div>
                    <div className="text-muted text-sm">{user ? "Kattints a képre a feltöltéshez" : "Bejelentkezés szükséges"}</div>
                    {avatarUrl && <div style={{fontSize:"0.75rem",color:"var(--neon)",marginTop:"0.2rem"}}>✅ Kép feltöltve</div>}
                  </div>
                </div>
                <div className="grid-2">
                  <div className="form-group">
  ...
  return (
    <div className={`profile-card ${highlight ? "ai-pick" : ""}`} onClick={() => onClick(profile)}>
      ...
      <div className="profile-card-body">
        ...
        {/* Instagram / kapcsolatfelvétel gomb */}
        <div style={{marginTop:"0.75rem"}} onClick={e=>e.stopPropagation()}>
          {profile.instagram ? (
            ...
            <button
              ...
              style={{width:"100%",justifyContent:"center",fontSize:"0.82rem",padding:"0.5rem",opacity:0.4,cursor:"not-allowed"}}
              disabled
              title="Nincs megadott elérhetőség">
...
export default function GymBro() {
  ...
  const [myProfile, setMyProfile] = useState({
    nev: "", kor: "", megye: "", varos: "", gym: "",
    edzesTipus: [], eroSzint: "", napok: [], idopontok: [], leiras: "", instagram: ""
  });
  const [saved, setSaved] = useState(false);
  const [saveError, setSaveError] = useState("");
  const [avatarUrl, setAvatarUrl] = useState("");
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          const mapped = data.map(p => ({
            ...
            napok: p.napok || [], idopontok: p.idopontok || [], leiras: p.leiras,
            avatar: "👤", online: p.online ?? true, userId: p.user_id,
            avatarUrl: p.avatar_url || "", instagram: p.instagram || "", avatarUrl: p.avatar_url || ""
          }));
          setProfiles([...mapped, ...DEMO_PROFILES]);
          // Ha be van jelentkezve, megkeressük a saját profil id-jét
          const session = await supabase.auth.getSession();
  ...
  async function saveProfile() {
    ...
    try {
      const { data, error } = await supabase.from("profiles").insert([{
        ...
        napok: myProfile.napok, idopontok: myProfile.idopontok,
        leiras: myProfile.leiras, online: true, user_id: user.id, avatar_url: avatarUrl || null,
        instagram: myProfile.instagram || null
      }]).select().single();
      if (error) throw error;
      const newP = { ...myProfile, id: data.id, kor: parseInt(myProfile.kor)||25, avatar: "👤", online: true, userId: user.id, avatarUrl: avatarUrl || "", instagram: myProfile.instagram || "" };
      setProfiles(prev => [newP, ...prev]);
      setMyProfileId(data.id);
      setSaved(true);
      setTimeout(() => setSaved(false), 3000);
            ...
            <>
              ...
              <div className="card">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="pl. @felhasznalonev vagy https://instagram.com/..."
                    value={myProfile.instagram} onChange={e=>setMyProfile(p=>({...p,instagram:e.target.value}))} />
                  <span style={{fontSize:"0.75rem",color:"var(--muted)"}}>Ha megadod, mások közvetlenül megtalálnak Instagramon</span>
                </div>
