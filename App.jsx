...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .story-ring-seen { background: conic-gradient(#444, #555, #444); }
  ...
  @keyframes glowPulse { 0%,100%{box-shadow:0 0 20px rgba(200,242,48,0.3)} 50%{box-shadow:0 0 50px rgba(200,242,48,0.7)} }
  .match-popup { animation: matchPop 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards; }
  .match-shake { animation: matchShake 0.5s ease; }
...
function StoryBar({ user, myProfileId, profiles, onLogin }) {
  ...
  async function uploadStory(){
    if(!uploadFile||!user||!myProfileId){return;}
    setUploading(true);
    try {
      ...
      await supabase.from("stories").insert([{
        user_id:user.id, profile_id:myProfileId,
        media_url:urlData.publicUrl, media_type:"image",
        caption:caption||null, story_type:storyType,
  ...
  return (
    <>
      ...
      <div style={{overflowX:"auto",display:"flex",gap:"0.85rem",padding:"0.75rem 0 0.75rem",marginBottom:"1.25rem",scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
        {/* Saját story feltöltés */}
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0.3rem",flexShrink:0,cursor:"pointer"}} onClick={()=>user?fileRef.current.click():onLogin()}>
          <div style={{width:60,height:60,borderRadius:"50%",border:"2px dashed var(--neon)",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--neon-dim)",position:"relative",overflow:"hidden"}}>
            {myStories?.length>0?(
              <img src={myStories[0].media_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:"50%"}}/>
            ):(
              <span style={{fontSize:"1.5rem",color:"var(--neon)"}}>+</span>
            )}
          </div>
          <span style={{fontSize:"0.68rem",color:"var(--muted)",maxWidth:60,textAlign:"center",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>Te</span>
        </div>
        ...
        {userIds.filter(uid=>uid!==user?.id).map(uid=>{
          ...
          return (
            <div key={uid} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"0.3rem",flexShrink:0,cursor:"pointer"}} onClick={()=>openStories(uid)}>
              <div className={seen?"story-ring-seen":"story-ring"} style={{width:64,height:64,borderRadius:"50%",padding:2.5,flexShrink:0}}>
                <div style={{width:"100%",height:"100%",borderRadius:"50%",overflow:"hidden",border:"2px solid var(--bg)",background:"var(--surface2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.4rem",position:"relative"}}>
                  {p?.avatarUrl?<img src={p.avatarUrl} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:p?.avatar||"👤"}
                  {sType&&<div style={{position:"absolute",bottom:0,right:0,fontSize:"0.7rem",background:"rgba(0,0,0,0.7)",borderRadius:"50%",width:18,height:18,display:"flex",alignItems:"center",justifyContent:"center"}}>{sType.icon}</div>}
                </div>
              </div>
              <span style={{fontSize:"0.68rem",color:seen?"var(--muted)":"var(--text)",maxWidth:60,textAlign:"center",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",fontWeight:seen?400:600}}>
                {p?.nev?.split(" ")[0]||"User"}
              </span>
            </div>
          );
