...
// ── Components ────────────────────────────────────────────────────────────────
function ProfileCard({ profile, highlight, aiNote, onClick, isOwn, isMatched, onMatch }) {
...
export default function GymBro() {
  ...
  const [myMatches, setMyMatches] = useState([]); // profile id-k amiket már matcheltem
  ...
  useEffect(()=>{
    supabase.auth.getSession().then(({data})=>{
      ...
      if(data.session?.user) loadMatches(data.session.user.id);
    });
            ...
            <>
              ...
              {myMatches.length>0&&(
                <div style={{background:"var(--surface)",border:"1px solid var(--border)",borderRadius:12,padding:"0.85rem 1.2rem",marginBottom:"1.25rem",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={()=>setFEdzTipus([])}>Mutat</button>
                </div>
              )}
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:"1rem"}}>
                {filtered.map(p=><ProfileCard key={p.id} profile={p} onClick={setSelected} isOwn={p.userId===user?.id} isMatched={myMatches.includes(p.id)} onMatch={toggleMatch}/>)}
                {filtered.length===0&&(
            ...
            <>
              ...
              {aiResults.length>0&&(
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:"1rem"}}>
                    {aiResults.map(({profile,note})=><ProfileCard key={profile.id} profile={profile} highlight aiNote={note} onClick={setSelected} isOwn={profile.userId===user?.id} isMatched={myMatches.includes(profile.id)} onMatch={toggleMatch}/>)}
                  </div>
                </>
              )}
