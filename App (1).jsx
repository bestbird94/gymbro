        ...
        <div style={{display:"flex",alignItems:"center",gap:"0.8rem"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"0.5rem"}}>
              ...
              <span style={{color:"var(--muted)",fontSize:"0.8rem",flexShrink:0}}>{profile.kor} év</span>
            </div>
...
export default function GymBro() {
  ...
  // Combine db + demo profiles (no duplicates)
  const profiles = [...dbProfiles, ...DEMO_PROFILES];
...
A felhasználó: ${myProfile.nev} | ${myProfile.megye} | ${myProfile.edzesTipus.join(",")} | ${myProfile.eroSzint} | Napok: ${myProfile.napok.join(",")}
Partnerek:
${profiles.slice(0,20).map((p,i)=>`${i+1}. ${p.nev} | ${p.varos},${p.megye} | ${p.edzesTipus.join("/")} | ${p.eroSzint} | ${p.napok.join(",")}`).join("\n")}
Válaszolj CSAK JSON-ban: {"ajanlott_indexek":[1,2,3],"magyarazatok":{"1":"...","2":"...","3":"..."},"osszefoglalas":"..."}
Max 3 ajánlás, 1-alapú indexek.`;
      const resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:prompt}]})});
      const data=await resp.json();
      const text=data.content?.map(c=>c.text||"").join("")||"";
      const parsed=JSON.parse(text.replace(/```json|```/g,"").trim());
      setAiResults((parsed.ajanlott_indexek||[]).map(i=>({profile:profiles[i-1],note:parsed.magyarazatok?.[String(i)]||""})).filter(x=>x.profile));
      setAiSummary(parsed.osszefoglalas||"");
    } catch(e){setAiSummary("Hiba az AI ajánló betöltésekor.");}
    setAiLoading(false);
  }
  ...
  async function saveProfile(){
    ...
    setSaveError("");
    try {
      const payload={nev:myProfile.nev,kor:parseInt(myProfile.kor)||25,megye:myProfile.megye,varos:myProfile.varos,gym:myProfile.gym,edzes_tipus:myProfile.edzesTipus,ero_szint:myProfile.eroSzint,napok:myProfile.napok,idopontok:myProfile.idopontok,leiras:myProfile.leiras,online:true,user_id:user.id,avatar_url:avatarUrl||null,instagram:myProfile.instagram||null};
      if(myProfileId){
            ...
            <>
              ...
              {aiResults.length>0&&(
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:"1rem"}}>
                    {aiResults.map(({profile,note})=><ProfileCard key={profile.id} profile={profile} highlight aiNote={note} onClick={setSelected}/>)}
                  </div>
                </>
              )}
